...
function SmartLogo({
    ...
    return (
        <div style={{
            ...
            width: "fit-content", // Ensure it doesn't collapse to 0 in flex environments
            display: "flex",
            alignItems: align,
...
function L1({ textSize = 1.0, textStyle = "blocky", logoScale = 1.0, brand, text, img, logoSrc, logoHasBg, logoIsLight, primary, headlineColor, bgOpacity = 100, tintIndex = 0, photoFilter = "none" }: LayoutProps) {
    // Use smartly-selected panel color that contrasts with the logo
    const rawTint = brand.colors[tintIndex % brand.colors.length] ?? primary;
    const tintColor = pickBestPanelColor(null, logoIsLight ?? true, brand.colors, rawTint);
    const brandFont = getFont(brand.fonts?.[0]);
    const font = getHeadlineFont(textStyle, brand.domain);
    ...
    return (
        <div className="absolute inset-0 overflow-hidden bg-black">
            ...
            {/* Very clean solid right-side panel (No gradient) */}
            ...
            <div className="absolute inset-y-0 right-0 w-[550px] flex flex-col items-center justify-center p-12 z-10">
                <div className="flex flex-col items-center gap-8">
                    <SmartLogo src={logoSrc} hasBg={logoHasBg} isLight={logoIsLight} panelColor={tintColor} h={180} maxW={400} brandColors={brand.colors} scale={logoScale} />
                    {text && (
                        <p style={{
...
function BannerCanvas({
    ...
    if (tintIndex === -1 && layout !== 8) {
        if ((!logoIsLight && bgIsDark) || logoIsSimilar) {
            ...
            // Light logo on light background → Force DARK background
...
const THUMB_CONFIGS: { label: string; preview: (color: string, accent: string) => React.ReactNode }[] = [
    ...
    },
    {
        ...
        preview: (c) => (
            <div className="w-full h-full overflow-hidden" style={{ position: "relative", background: PHOTO }}>
                <div style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: "48%", background: c, opacity: 0.9 }} />
                <div style={{ position: "absolute", right: "8%", top: "20%", width: "28%", height: "60%", background: "rgba(255,255,255,0.25)", borderRadius: 1 }} />
            </div>
        ),
    },
];
...
function BuilderContent() {
    ...
    const uniMetadata = ALL_UNIVERSITIES.find(u => u.domain === "ucl.ac.uk");
                        ...
                        {brand ? (
                            <>
                                ...
                                <div className="space-y-8 flex flex-col pt-4 md:pt-0 pl-1">
                                    ...
                                    <div>
                                        <label className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Variants</label>
